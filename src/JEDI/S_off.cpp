...
S_off::S_off(double v, double v0, double delta_v, vector<double> dv_dx, vector<double> dv_dy, vector<double> dv_dz)
{
 ...
 d_Soff_dx=derivatives;
 d_Soff_dy=derivatives;
 d_Soff_dz=derivatives;
 dm_dx=derivatives;
 dm_dy=derivatives;
 dm_dz=derivatives;
...
/* 
This function computes m_value and its derivatives, which are then passed on to compute_S_off.
Since m_value is not used anywhere else in the code, this function is private to this class.
*/
void S_off::compute_m(double v, double v0, double delta_v, vector<double> dv_dx, vector<double> dv_dy, vector<double> dv_dz)
{
    m_value=(v-v0)/delta_v;

    if (m_value <= 0 or 1 <= m_value) return; // don't calculate derivatives if S_off is flat
...
void S_off::compute_S_off()
{
  ...
  else if (0<m_value and m_value<1) 
  {
    ...
    for (unsigned j=0; j<dm_dx.size();j++)
    {
        d_Soff_dx[j]=d_Soff_dm*dm_dx[j];
        d_Soff_dy[j]=d_Soff_dm*dm_dy[j];
        d_Soff_dz[j]=d_Soff_dm*dm_dz[j];
    }
