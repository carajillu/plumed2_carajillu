...
bool jediparameters::readParams(string &parameters_file)
{
  ...
  if (!fp)
  {
   //JCN Aug2019: FIXME: This should be rewritten as a plumed error, but couldn't make it work so far
   cout << "JEDI parameters file: " << parameters_file.c_str() << " not found. Exiting." << endl;
   exit(0);
  }
  string line;
